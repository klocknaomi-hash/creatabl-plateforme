...
export default function UpgradeRequiredPage() {
    ...
    if (planId === 'free') {
      try {
        ...
        if (res.ok) {
          ...
          throw new Error('Failed to init free')
        }
      } catch (err) {
  ...
  return (
    <div className="w-full max-w-[1400px] p-6 md:p-10 my-8 overflow-hidden">
      <div className="text-center mb-10 space-y-3">
        ...
        <p className="text-zinc-400 text-base max-w-xl mx-auto">
          Découvre nos abonnements ou continue gratuitement avec le plan Free (20 posts/mois).
        </p>
      </div>
      ...
      <div className="flex justify-center mb-10">
        <div className="bg-[#110c1d] p-1.5 rounded-full flex items-center gap-1">
          <button
            ...
            className={`px-8 py-2.5 rounded-full text-sm font-bold transition-all cursor-pointer ${
          ...
          <button
            ...
            className={`px-8 py-2.5 rounded-full text-sm font-bold transition-all cursor-pointer flex items-center gap-2 ${
        ...
        {PLANS.map((planItem) => {
          ...
          return (
            <motion.div
              key={planItem.id}
              whileHover={{ y: -4 }}
              className={`h-full flex flex-col p-8 bg-white rounded-3xl border transition-all ${
                planItem.recommended
                  ? 'border-[3px] border-[#8B5CF6] scale-105 z-10 shadow-2xl relative'
                  : 'border-zinc-800 shadow-sm hover:shadow-lg'
              }`}
            >
              {planItem.recommended && (
              ...
              <div className="mt-auto pt-8 space-y-2 text-center">
                <button
                  ...
                  className={`w-full py-4 px-4 rounded-full font-black text-sm flex items-center justify-center gap-2 transition-all shadow-lg active:scale-95 cursor-pointer ${planItem.buttonStyle} border-2 disabled:opacity-50 disabled:cursor-not-allowed`}
                >
                  {loading === planItem.id ? (
