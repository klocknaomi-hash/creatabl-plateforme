...
const STATUS_CONFIG: Record<string, { label: string, icon: any, color: string, badge: string }> = {
  ...
  failed: { label: "Failed", icon: XCircle, color: "text-destructive", badge: "bg-destructive text-white border-transparent" },
};
    ...
    <div className="flex-1 space-y-6 max-w-6xl mx-auto w-full">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Manage and track your content across all platforms.</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {data?.posts?.map((post: any) => {
            ...
            return (
              <Card key={post.id} className="group overflow-hidden border-border/50 hover:border-primary/40 hover:shadow-2xl hover:shadow-primary/5 transition-all duration-500 rounded-[32px] bg-card flex flex-col">
                  ...
                  <div className="absolute top-3 right-3">
                     <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="rounded-2xl border-border/50 shadow-2xl min-w-[160px] p-2">
                        <DropdownMenuItem onClick={() => (window.location.href = `/dashboard/compose?id=${post.id}`)} className="rounded-xl gap-2 h-10">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                       ...
                       {post.platforms.length > 1 && (
                         <span className="text-[10px] text-muted-foreground font-bold">+{post.platforms.length - 1} more</span>
                       )}
                    </div>
